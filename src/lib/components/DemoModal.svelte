    ...
    >
      ...
      {#if isSuccess}
        <div class="py-8 text-center flex flex-col items-center">
          ...
          <p class="mt-2 text-sm text-slate-500 max-w-sm">
            Thank you, <strong class="text-slate-800">{name}</strong>. We've sent a calendar invitation for <strong class="text-slate-850">{date} at {timeId}</strong> to <span class="text-blue-600 underline">{email}</span>.
          </p>
          <button
            onclick={resetAndClose}
      ...
      {:else}
        <div class="mb-6 flex items-start gap-3.5">
          <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-50 text-blue-650 shrink-0">
            <Calendar class="h-5.5 w-5.5" />
          </div>
          <div>
            <h3 class="text-lg font-extrabold text-slate-900 font-heading">Book a Live Demo</h3>
            <p class="text-sm text-slate-480 mt-0.5">Experience how Bomax ERP optimizes your glass business operations.</p>
          </div>
        </div>
        ...
        <form onsubmit={handleBook} class="space-y-4">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label for="modal-name" class="block text-xs font-bold text-slate-700 mb-1">Your Name</label>
              <input
                id="modal-name"
                type="text"
                bind:value={name}
            ...
            <div>
              <label for="modal-email" class="block text-xs font-bold text-slate-700 mb-1">Work Email</label>
              <input
                id="modal-email"
                type="email"
                bind:value={email}
          ...
          <div>
            <label for="modal-company" class="block text-xs font-bold text-slate-700 mb-1">Company Name</label>
            <input
              id="modal-company"
              type="text"
              bind:value={company}
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label for="modal-date" class="block text-xs font-bold text-slate-700 mb-1">Preferred Date</label>
              <input
                id="modal-date"
                type="date"
                bind:value={date}
            ...
            <div>
              <span class="block text-xs font-bold text-slate-705 mb-1">Best Time (EST)</span>
              <div class="relative">
                <Clock class="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none" />
                <select
                  id="modal-time"
                  bind:value={timeId}
          ...
          >
            {isSubmitting ? 'Scheduling Demo...' : 'Confirm Demo Schedule'}
          </button>
        </form>
      {/if}
    </div>
